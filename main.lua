...
-- ВРЕМЯ (Москва, UTC+3)
--------------------------------------------------
-- Смещение часового пояса в часах (Москва = 3). Живёт отдельной переменной,
-- т.к. таблица Settings объявляется ниже по файлу; синхронизируется из
-- настроек через setTimezone() при загрузке и при изменении в админке.
_tzOffset = 3
local function setTimezone(h)
    h = tonumber(h)
    if h and h >= -12 and h <= 14 then _tzOffset = h end
end

-- Реальное (не игровое) время. В OpenComputers os.time() возвращает время
-- игрового мира — отсюда в логах брались даты вроде 1985 года. Реальный
-- unix-timestamp даёт computer.realTime(); os.time() оставлен запасным
-- вариантом на случай, если версия OC его не поддерживает.
local function realNow()
    local t
    if computer.realTime then
        local ok, v = pcall(computer.realTime)
        if ok and type(v) == "number" and v > 1000000000 then
            -- некоторые сборки отдают миллисекунды
    ...
    if not t then
        ...
        if ok and type(v) == "number" and v > 1000000000 then t = math.floor(v) end
    end
    if not t then
        t = math.floor(computer.uptime() + 1700000000)
    end
    return t + math.floor((_tzOffset or 3) * 3600)
end

-- совместимость со старым именем
local function moscowNow() return realNow() end
...
function Settings.load()
    ...
    if loaded then
        ...
        Settings.data.winChance = tonumber(Settings.data.winChance)
            or (Settings.data.houseEdge and (50 - tonumber(Settings.data.houseEdge) / 2)) or 50
        Settings.data.houseEdge = nil
        Settings.data.sessionSeconds = tonumber(Settings.data.sessionSeconds) or 40
        Settings.data.tzOffset = tonumber(Settings.data.tzOffset) or 3
    ...
    setTimezone(Settings.data.tzOffset)
end
...
-- Подбрасывает монету с прямым шансом на угадывание (winChance %, 1-99).
-- Это не «перевес», а буквальная вероятность того, что выпадет сторона,
-- выбранная игроком — админ управляет сложностью напрямую и без иллюзий.
function Coin.flip(choice, winChance)
    winChance = tonumber(winChance) or 50
    if winChance < 1 then winChance = 1 end
    if winChance > 99 then winChance = 99 end
...
function Game.decide(choice)
    Game.choice = choice
    local chance = tonumber(Settings.data.winChance) or 50
    Game.result = Coin.flip(choice, chance)
...
-- Найти уже нарисованную кнопку по подписи (для частичной перерисовки)
function UI.findButton(label)
...
function UI.drawButton(b)
    ...
    -- Точный центр для нечётной высоты (h=3,5 → ровно середина).
    -- Все кнопки в интерфейсе сделаны нечётной высоты именно поэтому:
    -- при чётной высоте математического центра не существует и текст
    -- неизбежно уезжает выше или ниже середины.
    local ty = b.y + math.floor((b.h - 1) / 2)
    gpu.setForeground(b.fg); gpu.setBackground(b.bg)
    gpu.set(tx, ty, label)
end
...
-- Фон стола рисуется напрямую (заливка + тонкая рамка = несколько вызовов
-- gpu.fill). Отдельный буфер-кэш под него больше не держим: видеопамяти GPU
-- хватает не на два полноэкранных буфера, и когда выделение буфера кадра
-- падало, вся отрисовка шла напрямую на экран мимо двойной буферизации —
-- это и давало мигание при каждом нажатии кнопки.
...
function blitTable(mw)
    ...
    -- Рисуем фон в ТЕКУЩИЙ активный буфер. Раньше здесь безусловно
    -- переключались на экран (setActiveBuffer 0), что рвало кадр, если
    -- функцию вызывали изнутри двойной буферизации.
...
function drawTableRail(mw) paintRailToActive(mw) end
...
function drawScreen()
    ...
    -- Двойная буферизация (если GPU её поддерживает): кадр собирается вне
    -- экрана и выводится одним bitblt. На старых версиях OpenComputers
    -- буферов нет — тогда рисуем напрямую, а мелкие изменения (выбор стороны,
    -- сумма ставки) идут через UI.setChoice/UI.setBet без полной перерисовки.
    beginFrame()

    if isAdmin or UI.alert or UI.input.active or UI.withdraw.active then
        -- модальные экраны: ровный фон без бортика стола
        fill(1, 1, mw, UI.h, FELT_BASE)
    else
        -- фон стола рисуем прямо в активный буфер кадра
        paintFeltToActive(mw, UI.h)
        paintRailToActive(mw)
    end
...
local COIN_W, COIN_H = 23, 11
local COIN_Y = 5  -- небольшой отступ от бортика стола, чтобы не липла к рамке
local COIN_GOLD  = 0xEACB6A
local COIN_RIM   = 0xA9822C
local STEP_BTN_BG = 0x3C3C3C  -- кнопки быстрой ставки — заметно светлее тёмного стола
local FLIP_MAX_DY = 2         -- максимальное смещение монеты вниз в анимации
local FELT_TOP = 4  -- первая строка «сукна» под бортиком — ниже не стираем
...
-- Считаем маску с виртуальным запасом в 1 строку сверху/снизу и обрезаем
-- эти строки — иначе самая верхняя/нижняя точка эллипса вырождается
-- в одинокий символ шириной 1 и выглядит как «шип», торчащий из монеты.
local function buildEllipseMask(w, h, shrink)
    shrink = shrink or 0
    local shape = {}
    local rx = (w - 1) / 2 - shrink
    local virtualH = h + 2
    local ry = (virtualH - 1) / 2 - shrink
    local cyc = (virtualH + 1) / 2
    for row = 1, h do
        local vrow = row + 1
        local hw = 0
        if ry > 0 then
            local ny = (vrow - cyc) / ry
...
-- Тонкий ободок (1 клетка по контуру), а не полосы через центр: клетка —
-- «край», если хотя бы один из соседей (сверху/снизу/слева/справа) не
-- закрашен. Даёт эффект чеканной монеты без «линий» посередине.
local function buildRimMask(shape, w, h)
    local rim = {}
    local function filled(r, c)
        if r < 1 or r > h or c < 1 or c > w then return false end
        return shape[r]:sub(c, c) == "#"
    end
    for row = 1, h do
        local out = {}
        for col = 1, w do
            if filled(row, col) then
                local edge = (not filled(row - 1, col)) or (not filled(row + 1, col))
                    or (not filled(row, col - 1)) or (not filled(row, col + 1))
                out[col] = edge and "E" or "I"
            else
                out[col] = "."
            end
        end
        rim[row] = table.concat(out)
    end
    return rim
end
local COIN_RIM_MASK = buildRimMask(COIN_SHAPE, COIN_W, COIN_H)
...
function UI.coinPos()
    ...
    return cx - math.floor(COIN_W / 2), COIN_Y, mw, cx
end
...
-- Стирает область с запасом сверху/снизу — используется во время анимации,
-- когда монета слегка «подпрыгивает» по вертикали (см. UI.startFlipAnim).
-- Верх обрезан по FELT_TOP, чтобы стирание никогда не «съедало» бортик
-- стола (иначе во время анимации на миг становится виден край сукна поверх
-- рамки — та самая «прыгающая рамка»).
function eraseCoinArea(x, y)
    gpu.setBackground(FELT_BASE)
    local top = math.max(y, FELT_TOP)
    local bottom = y + COIN_H - 1 + FLIP_MAX_DY
    gpu.fill(x, top, COIN_W, bottom - top + 1, " ")
end
...
function drawCoin(x, y, w, side)
    ...
    for row = 1, COIN_H do
        ...
        local rim = COIN_RIM_MASK[row]
        for col = left, right do
            if line:sub(col, col) == "#" then
                -- срез видимого окна (во время вращения) тоже читается как край
                local isEdge = (rim:sub(col, col) == "E") or col == left or col == right
                gpu.setForeground(isEdge and COIN_RIM or COIN_GOLD)
                gpu.set(x + col - 1, y + row - 1, "█")
            end
        end
    end
    if side and w >= math.floor(COIN_W * 0.6) then
        text(x + math.floor((COIN_W - unicode.len(side)) / 2), y + math.floor(COIN_H / 2), side, 0x3A2A00, COIN_GOLD)
    end
end

--------------------------------------------------
-- ЧАСТИЧНАЯ ПЕРЕРИСОВКА
-- Полный UI.draw() очищает игровую зону и собирает её заново. Если GPU не
-- поддерживает буферы кадра (старые версии OpenComputers), эта очистка видна
-- глазом как вспышка при каждом нажатии. Поэтому действия, меняющие только
-- пару элементов (выбор стороны, сумма ставки), перерисовывают именно эти
-- элементы, а не весь экран.
--------------------------------------------------
function UI.setChoice(side)
    UI.choice = side
    if UI.screen ~= "main" or UI.alert or UI.input.active or UI.withdraw.active then
        UI.draw(); return
    end
    local orel = UI.findButton("ОРЁЛ")
    local reshka = UI.findButton("РЕШКА")
    if not (orel and reshka) then UI.draw(); return end

    orel.bg   = (side == "ОРЁЛ")  and config.colors.buttonBlue or config.colors.button
    reshka.bg = (side == "РЕШКА") and config.colors.buttonBlue or config.colors.button

    pcall(gpu.setActiveBuffer, 0)
    UI.drawButton(orel)
    UI.drawButton(reshka)
    local coinX, coinY = UI.coinPos()
    eraseCoin(coinX, coinY)
    drawCoin(coinX, coinY, COIN_W, UI.choice)
end

function UI.setBet(n)
    n = math.floor(tonumber(n) or UI.betAmount)
    local minB = tonumber(Settings.data.minBet) or 1
    local maxB = tonumber(Settings.data.maxBet) or 1000
    if n < minB then n = minB end
    if n > maxB then n = maxB end
    UI.betAmount = n

    local box = UI.betBox
    if UI.screen ~= "main" or UI.alert or UI.input.active or UI.withdraw.active or not box then
        UI.draw(); return
    end
    pcall(gpu.setActiveBuffer, 0)
    fill(box.x + 1, box.y + 1, box.w - 2, 1, config.colors.panelLight)
    text(box.x + 2, box.y + 1, tostring(n) .. " " .. config.currency.symbol,
        config.colors.textGold, config.colors.panelLight)
end
...
function UI.drawHistoryList(x, y, mw, maxRows)
    ...
    text(x + 2, y, " История ", config.colors.textBlue, config.colors.panel)
...
function UI.drawInputModal()
    ...
    UI.addButton(bx + 2, by + 7, 12, 3, "ОК", config.colors.buttonGreen, 0xFFFFFF, function() UI.closeInput(true) end)
    UI.addButton(bx + 16, by + 7, 12, 3, "ОТМЕНА", config.colors.button, config.colors.text, function() UI.closeInput(false) end)
end
...
function UI.drawSidebar()
    ...
    if not UI.authorized then
        UI.addButton(sx + 1, 3, sw - 2, 5, "АВТОРИЗАЦИЯ", config.colors.buttonGreen, 0xFFFFFF, function()
            UI.pendingAuth = true
            UI.setMessage("Коснитесь экрана своим ником", config.colors.textGold, 5)
            UI.draw()
        end)
        text(sx + 2, 10, "Нажмите кнопку", config.colors.textDark, config.colors.panel)
        text(sx + 2, 11, "для авторизации", config.colors.textDark, config.colors.panel)
    else
        UI.addButton(sx + 1, 3, sw - 2, 3, "ВЫХОД", config.colors.buttonRed, 0xFFFFFF, function() UI.logout() end)

        drawBox(sx + 1, 7, sw - 2, 5, config.colors.textDark, config.colors.panelLight)
        text(sx + 3, 8, UI.playerName or "?", config.colors.textGreen, config.colors.panelLight)
        local p = Players.get(UI.playerName)
        text(sx + 3, 9, string.format("%s %s", fmtMoney(p.balance or 0), config.currency.symbol), config.colors.textGold, config.colors.panelLight)
        text(sx + 3, 10, "Выход через: " .. UI.sessionLeft .. "с", config.colors.textDark, config.colors.panelLight)

        -- Пополнить + Вывод — на одном уровне, впритык, как одна кнопка из двух половин
        local halfW = math.floor((sw - 2) / 2)
        UI.addButton(sx + 1, 13, halfW, 3, "ПОПОЛНИТЬ", config.colors.buttonGreen, 0xFFFFFF, function() UI.doDeposit() end)
        UI.addButton(sx + 1 + halfW, 13, (sw - 2) - halfW, 3, "ВЫВОД", config.colors.buttonBlue, 0xFFFFFF, function() UI.doWithdraw() end)

        if config.admins[UI.playerName] then
            UI.addButton(sx + 1, 17, sw - 2, 3, "АДМИН ПАНЕЛЬ", config.colors.buttonBlue, 0xFFFFFF, function()
                UI.screen = "admin"; UI.adminTab = "bets"; UI.draw()
            end)
        end
    end

    local y = UI.authorized and (config.admins[UI.playerName] and 22 or 18) or 14
...
function UI.drawMainArea()
    local coinX, coinY, mw, cx = UI.coinPos()

    -- История прижата к нижнему-левому углу СТОЛА (сукна), а не экрана:
    -- отступ на 1 клетку от бортика слева и снизу. Позиция считается от
    -- размеров стола, поэтому блок не «съезжает» между экранами.
    local function drawHistoryAtTableCorner()
        local n = #History.entries
        local rows = math.min(10, n)
        local boxH = (n == 0) and 5 or (rows + 4)
        local feltBottom = UI.h - 2          -- последняя строка сукна (ниже — бортик)
        local y = feltBottom - boxH          -- +1 клетка воздуха до бортика
        local minY = coinY + COIN_H + 2
        if y < minY then
            -- на низком экране ужимаем количество строк, а не наезжаем на монету
            rows = math.max(3, rows - (minY - y))
            boxH = rows + 4
            y = math.max(minY, feltBottom - boxH)
        end
        UI.drawHistoryList(4, y, mw, rows)
    end

    if UI.screen == "main" then
        drawCoin(coinX, coinY, COIN_W, UI.choice)

        local infoY = coinY + COIN_H + 2
        ...
        local sideLabelY = infoY + 1
        centerText(sideLabelY, "ВЫБЕРИТЕ СТОРОНУ", config.colors.textBlue, config.colors.background, mw)
        local sideY = sideLabelY + 1
        ...
        UI.addButton(cx - 21, sideY, 20, 3, "ОРЁЛ", orelBg, 0xFFFFFF, function() UI.setChoice("ОРЁЛ") end)
        UI.addButton(cx + 1, sideY, 20, 3, "РЕШКА", reshkaBg, 0xFFFFFF, function() UI.setChoice("РЕШКА") end)

        local betLabelY = sideY + 4
        ...
        UI.betBox = { x = cx - 10, y = betBoxY, w = 20, h = 3 }
        ...
        UI.addButton(cx - 10, betBoxY, 20, 3, "", 0x000000, 0x000000, function()
            UI.openInput("Ставка (целое число)", tostring(UI.betAmount), function(val)
                ...
                UI.betAmount = n
                UI.draw()   -- после модалки экран всё равно перерисовывается целиком
            end, 8)
        end)
        ...
        -- Рамка вокруг всего ряда быстрых кнопок — визуально объединяет их
        -- в один элемент управления и отделяет от тёмного фона стола
        drawBox(stepX - 2, stepsY - 1, totalW + 4, stepH + 2, config.colors.textDark, config.colors.background)

        for i, s in ipairs(steps) do
            local bx = stepX + (i - 1) * (stepW + gap)
            UI.addButton(bx, stepsY, stepW, stepH, s.label, STEP_BTN_BG, 0xFFFFFF, function()
                UI.setBet(s.fn())
            end)
        end

        local limitY = stepsY + stepH + 2
        ...
        drawHistoryAtTableCorner()

    elseif UI.screen == "playing" or UI.screen == "result" then
        if UI.anim then
            drawCoin(coinX, coinY + (UI.anim.dy or 0), UI.anim.w, UI.anim.side)
        elseif UI.screen == "result" then
            drawCoin(coinX, coinY, COIN_W, Game.result)
        end

        -- Отступ учитывает максимальное смещение монеты вниз в анимации,
        -- иначе на пиковом кадре монета (и её стирание) залезает прямо
        -- на строку с текстом «Ваш выбор / Ставка» и рвёт её на части.
        local infoY = coinY + COIN_H + FLIP_MAX_DY + 1
        ...
        elseif UI.screen == "result" then
            ...
            if winAmount > 0 then
                centerText(statusY + 1, "+" .. winAmount .. " " .. config.currency.symbol .. " на баланс", config.colors.textGreen, config.colors.background, mw)
            end
...
function UI.drawAdmin(mw)
    ...
    for _, tab in ipairs(tabs) do
        ...
        UI.addButton(tx, 5, tw, 3, tab.title,
    ...
    UI.addButton(4, UI.h - 4, 14, 3, "◄ НАЗАД", config.colors.button, config.colors.text, function()
...
function UI.drawAdminBuy(mw)
    ...
    UI.addButton(4, UI.h - 8, 22, 3, "+ ДОБАВИТЬ ПРЕДМЕТ", config.colors.buttonGreen, 0xFFFFFF, function()
...
function UI.drawAdminPayout(mw)
    text(4, 9, "Предмет выигрыша / вывода:", config.colors.textBlue, config.colors.background)
    text(4, 10, "(выдаётся из ME в правый сундук)", config.colors.textDark, config.colors.background)
    ...
    if pi and pi.name then
        ...
        UI.addButton(4, 17, 18, 3, "ИЗМЕНИТЬ", config.colors.buttonBlue, 0xFFFFFF, function()
            UI.screen = "admin_add_item"
            UI.editItem = { name = pi.name, label = pi.label or pi.name, price = tostring(pi.value or 1), mode = "edit", target = "payout" }
            UI.draw()
        end)
        UI.addButton(24, 17, 14, 3, "УДАЛИТЬ", config.colors.buttonRed, 0xFFFFFF, function()
        ...
        UI.addButton(4, 17, 24, 3, "+ НАСТРОИТЬ ПРЕДМЕТ", config.colors.buttonGreen, 0xFFFFFF, function()
            UI.screen = "admin_add_item"
...
function UI.drawAdminLogs(mw)
    text(4, 9, "Последние действия:", config.colors.textBlue, config.colors.background)
    local y = 11
    ...
    UI.addButton(20, UI.h - 4, 5, 3, "▲", config.colors.button, config.colors.text, function()
        UI.logScroll = math.max(0, (UI.logScroll or 0) - 2); UI.draw()
    end)
    UI.addButton(26, UI.h - 4, 5, 3, "▼", config.colors.button, config.colors.text, function()
...
function UI.drawAdminOdds(mw)
    text(4, 9, "Выплата:", config.colors.textBlue, config.colors.background)

    text(4, 11, "Угадал x:", config.colors.textDark, config.colors.background)
    drawBox(4, 12, 12, 3, config.colors.textGold, config.colors.panelLight)
    text(6, 13, tostring(Settings.data.winPayout or 2.0), config.colors.textGold, config.colors.panelLight)
    UI.addButton(4, 12, 12, 3, "", 0x000000, 0x000000, function()
    ...
    text(4, 16, "Сложность:", config.colors.textRed, config.colors.background)
    text(4, 18, "Шанс игрока на победу, % (5-95):", config.colors.textDark, config.colors.background)
    drawBox(4, 19, 12, 3, config.colors.textGold, config.colors.panelLight)
    text(6, 20, tostring(Settings.data.winChance or 50), config.colors.textGold, config.colors.panelLight)
    UI.addButton(4, 19, 12, 3, "", 0x000000, 0x000000, function()
        UI.openInput("Шанс победы %", tostring(Settings.data.winChance or 50), function(val)
            local n = tonumber(val)
            if n then n = math.floor(n); if n >= 5 and n <= 95 then Settings.data.winChance = n; Settings.save() end end
            UI.draw()
        end, 2)
    end)
    text(4, 23, "(прямая вероятность угадывания — 50 = честно,", config.colors.textDark, config.colors.background)
    text(4, 24, "ниже — сложнее для игрока)", config.colors.textDark, config.colors.background)

    text(4, 27, "Сессия:", config.colors.textBlue, config.colors.background)
    text(4, 29, "Автовыход через, сек:", config.colors.textDark, config.colors.background)
    drawBox(4, 30, 12, 3, config.colors.textGold, config.colors.panelLight)
    text(6, 31, tostring(Settings.data.sessionSeconds or 40), config.colors.textGold, config.colors.panelLight)
    UI.addButton(4, 30, 12, 3, "", 0x000000, 0x000000, function()
    ...
    text(4, 34, "(10-600 секунд бездействия)", config.colors.textDark, config.colors.background)

    text(4, 37, "Время в логах:", config.colors.textBlue, config.colors.background)
    text(4, 39, "Часовой пояс UTC+ (Москва = 3):", config.colors.textDark, config.colors.background)
    drawBox(4, 40, 12, 3, config.colors.textGold, config.colors.panelLight)
    local tzVal = tonumber(Settings.data.tzOffset) or 3
    text(6, 41, (tzVal >= 0 and "+" or "") .. tostring(tzVal), config.colors.textGold, config.colors.panelLight)
    UI.addButton(4, 40, 12, 3, "", 0x000000, 0x000000, function()
        UI.openInput("Часовой пояс UTC+", tostring(tonumber(Settings.data.tzOffset) or 3), function(val)
            local n = tonumber(val)
            if n and n >= -12 and n <= 14 then
                Settings.data.tzOffset = n
                setTimezone(n)
                Settings.save()
            end
            UI.draw()
    ...
    text(4, 44, "Сейчас: " .. moscowDate("%Y-%m-%d %H:%M:%S"), config.colors.textDark, config.colors.background)
end
...
function UI.updateSessionLabel()
    ...
    gpu.fill(sx + 2, 10, sw - 4, 1, " ")
    gpu.setForeground(config.colors.textDark)
    gpu.set(sx + 3, 10, "Выход через: " .. tostring(UI.sessionLeft) .. "с")
end
...
-- Смещения только ВНИЗ: вверх монета уезжала на бортик стола и «прилипала»
-- к рамке, потому что COIN_Y уже стоит вплотную к верхней границе сукна.
local FLIP_OFFSETS   = { 0,   0,    1,    1,    2,    1,    1,    0  }
...
function UI.startFlipAnim()
    ...
    for spin = 1, spins do
        ...
        for idx, f in ipairs(FLIP_FRACTIONS) do
            local w = math.max(3, math.floor(COIN_W * f + 0.5))
            table.insert(frames, { w = w, side = (f >= 0.6) and faceSide or nil, dy = FLIP_OFFSETS[idx] })
        end
    end
    table.insert(frames, { w = COIN_W, side = Game.result, dy = 0 })
    ...
    local function step()
        ...
        UI.anim = frames[i]
        -- Кадр анимации рисуем ВНЕ экрана (в буфер) и переносим готовым
        -- одним bitblt. Иначе последовательность «стереть → нарисовать»
        -- выполняется прямо на экране и на кадр видна пустая область —
        -- это и читается как мигание монеты.
        local region = { x = coinX, y = COIN_Y, w = COIN_W, h = COIN_H + FLIP_MAX_DY }
        local drewBuffered = false
        if _screenBuf and gpu.setActiveBuffer and gpu.bitblt then
            if pcall(gpu.setActiveBuffer, _screenBuf) then
                eraseCoinArea(coinX, coinY)
                drawCoin(coinX, coinY + (UI.anim.dy or 0), UI.anim.w, UI.anim.side)
                pcall(gpu.setActiveBuffer, 0)
                drewBuffered = pcall(gpu.bitblt, 0, region.x, region.y, region.w, region.h,
                    _screenBuf, region.x, region.y)
            end
        end
        if not drewBuffered then
            -- fallback без буфера (старый GPU): рисуем напрямую
            pcall(gpu.setActiveBuffer, 0)
            eraseCoinArea(coinX, coinY)
            drawCoin(coinX, coinY + (UI.anim.dy or 0), UI.anim.w, UI.anim.side)
        end
        UI.schedule(0.055, step)
    end
    step()
end
...
function UI.resolveGame()
    ...
    if Game.win then
        -- Выигрыш начисляется на баланс. Предметами игрок забирает его сам
        -- через кнопку «ВЫВОД» — так выигрыш не зависит от наличия предмета
        -- в ME и от свободного места в сундуке в момент броска.
        if winAmount > 0 then
            Players.addBalance(UI.playerName, winAmount)
            log("ВЫИГРЫШ", UI.playerName, string.format(
                "Ставка %d | выпало %s | +%d %s на баланс",
                Game.bet, tostring(Game.result), winAmount, config.currency.symbol
            ))
        end
    else
