...
function drawScreen()
    ...
    -- ВАЖНО: кэш фона готовим ДО начала кадра. ensureTableCache/blitTable
    -- переключают активный буфер на 0, и если вызвать их внутри кадра,
    -- вся дальнейшая отрисовка пойдёт напрямую на экран мимо буфера —
    -- пользователь видит, как интерфейс собирается по частям (мигание).
    beginFrame()
