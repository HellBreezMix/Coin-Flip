...
function UI.drawButton(b)
    ...
    -- центр по вертикали: для чётной высоты кнопки округляем вниз
    -- (иначе текст визуально «прилипает» к верхнему краю)
    local ty = b.y + math.ceil((b.h - 1) / 2)
...
function eraseCoinArea(x, y)
    gpu.setBackground(FELT_BASE)
    local top = math.max(y - 2, FELT_TOP)
    local bottom = y + COIN_H + 1
...
function UI.drawMainArea()
    ...
    -- История всегда на одном и том же месте, независимо от экрана —
    -- иначе блок «прыгает» при переходах main → playing → result.
    local HISTORY_Y = coinY + COIN_H + 24  -- держать в синхроне с actionY ниже
    ...
    if UI.screen == "main" then
        ...
        local maxRows = math.max(3, math.min(10, UI.h - HISTORY_Y - 3))
        UI.drawHistoryList(4, HISTORY_Y, mw, maxRows)
...
function UI.drawAdminOdds(mw)
    ...
    text(4, 14, "Сложность:", config.colors.textRed, config.colors.background)
    text(4, 16, "Шанс игрока на победу, % (5-95):", config.colors.textDark, config.colors.background)
    drawBox(4, 17, 12, 3, config.colors.textGold, config.colors.panelLight)
    text(6, 18, tostring(Settings.data.winChance or 50), config.colors.textGold, config.colors.panelLight)
    ...
    text(4, 21, "(прямая вероятность угадывания — 50 = честно,", config.colors.textDark, config.colors.background)
    text(4, 22, "ниже — сложнее для игрока)", config.colors.textDark, config.colors.background)

    text(4, 26, "Сессия:", config.colors.textBlue, config.colors.background)
    text(4, 28, "Автовыход через, сек:", config.colors.textDark, config.colors.background)
    drawBox(4, 29, 12, 3, config.colors.textGold, config.colors.panelLight)
    text(6, 30, tostring(Settings.data.sessionSeconds or 40), config.colors.textGold, config.colors.panelLight)
    UI.addButton(4, 29, 12, 3, "", 0x000000, 0x000000, function()
    ...
    text(4, 33, "(10-600 секунд бездействия)", config.colors.textDark, config.colors.background)
end
