...
local COIN_W, COIN_H = 25, 9
local COIN_GOLD  = 0xD4AF37
local COIN_GOLD_DARK = 0x9C7A1E
...
local function buildCoinShape(w, h)
    local shape = {}
    for row = 1, h do
        local edge = math.min(row - 1, h - row)
        local pad
        if edge == 0 then pad = math.floor(w * 0.22 + 0.5)
        elseif edge == 1 then pad = math.floor(w * 0.06 + 0.5)
        else pad = 0 end
        local hashLen = w - pad * 2
        shape[row] = string.rep(".", pad) .. string.rep("#", hashLen) .. string.rep(".", pad)
    end
    return shape
end
local COIN_SHAPE = buildCoinShape(COIN_W, COIN_H)
...
function drawCoin(x, y, w, side)
    ...
    -- тонкий внутренний кант — только когда монета почти развёрнута
    if w >= COIN_W - 4 then
        gpu.setForeground(COIN_GOLD_DARK)
        for col = left + 2, left + w - 3 do
            if COIN_SHAPE[2]:sub(col, col) == "#" then gpu.set(x + col - 1, y + 1, "▄") end
            if COIN_SHAPE[COIN_H - 1]:sub(col, col) == "#" then gpu.set(x + col - 1, y + COIN_H - 2, "▀") end
        end
    end
    if side and w >= math.floor(COIN_W * 0.55) then
        text(x + math.floor((COIN_W - unicode.len(side)) / 2), y + math.floor(COIN_H / 2), side, 0x3A2A00, COIN_GOLD)
    end
    -- уголки-ромбики, как на игральных картах — только при полностью развёрнутой монете
    if w >= COIN_W then
        gpu.setForeground(COIN_GOLD)
        gpu.setBackground(FELT_BASE)
        gpu.set(x - 2, y + 1, "◆")
        gpu.set(x + COIN_W + 1, y + 1, "◆")
        gpu.set(x - 2, y + COIN_H - 2, "◆")
        gpu.set(x + COIN_W + 1, y + COIN_H - 2, "◆")
    end
end

-- Таблица «История флипов» — как в казино: результат, выбор > выпало, сумма
function UI.drawHistoryList(x, y, mw, maxRows)
    local n = #History.entries
    text(x, y, "История флипов:", config.colors.textBlue, FELT_BASE)
    y = y + 1
    fill(x, y, math.min(mw - x - 2, 46), 1, FELT_BASE)
    gpu.setForeground(0x1A5A38); gpu.setBackground(FELT_BASE)
    for i = 0, math.min(mw - x - 3, 45) do gpu.set(x + i, y, "─") end
    y = y + 1
    if n == 0 then
        text(x, y, "Пока нет результатов", config.colors.textDark, FELT_BASE)
        return
    end
    local rows = math.min(maxRows or 10, n)
    for i = 1, rows do
        local e = History.entries[i]
        local ry = y + i - 1
        local tag = e.win and "WIN" or "---"
        local tagCol = e.win and config.colors.textGreen or config.colors.textDark
        ...
        text(x, ry, tag, tagCol, FELT_BASE)
        text(x + 5, ry, tostring(e.choice), choiceCol, FELT_BASE)
        text(x + 12, ry, ">" .. tostring(e.result), resultCol, FELT_BASE)
        text(x + 22, ry, sign .. tostring(e.amount or 0) .. " " .. config.currency.symbol, amtCol, FELT_BASE)
    end
end
