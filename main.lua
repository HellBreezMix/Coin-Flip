...
local COIN_GOLD_RIM = 0x9C7A1E
local COIN_TEXT  = 0x3A2A00
...
local COIN_INNER = buildEllipseMask(COIN_W, COIN_H, 2)  -- внутренний диск (светлее — эффект объёма)
...
function drawCoin(x, y, w, side)
    ...
    gpu.setBackground(FELT_BASE)
    for row = 1, COIN_H do
        local outer = COIN_SHAPE[row]
        local inner = COIN_INNER[row]
        for col = left, left + w - 1 do
            if outer:sub(col, col) == "#" then
                gpu.setForeground(inner:sub(col, col) == "#" and COIN_GOLD or COIN_GOLD_RIM)
                gpu.set(x + col - 1, y + row - 1, "█")
            end
        end
    end
    if side and w >= math.floor(COIN_W * 0.6) then
        text(x + math.floor((COIN_W - unicode.len(side)) / 2), y + math.floor(COIN_H / 2), side, COIN_TEXT, COIN_GOLD)
    end
end
...
function UI.drawHistoryList(x, y, mw, maxRows)
    ...
    text(x, y, "История:", config.colors.textBlue, FELT_BASE)
    y = y + 1
    local lineW = math.max(0, math.min(mw - x - 2, 46))
    gpu.setForeground(TABLE_RAIL); gpu.setBackground(FELT_BASE)
    for i = 0, lineW - 1 do gpu.set(x + i, y, "─") end
    y = y + 1
    if n == 0 then
...
function UI.drawSidebar()
    ...
    if not UI.authorized then
        ...
        UI.addButton(sx + 1, 15, sw - 2, 3, "ВЫВОД", config.colors.buttonBlue, 0xFFFFFF, function() UI.doWithdraw() end)
    ...
    local y = UI.authorized and (config.admins[UI.playerName] and 23 or 19) or 12
...
function UI.doWithdraw()
    ...
    UI.openInput("Сумма вывода (" .. config.currency.symbol .. ")", "", function(val)
        if not val or val == "" then UI.draw(); return end
        local n = tonumber(val)
        if not n or n <= 0 then
            UI.setMessage("Введите положительное число", config.colors.textRed, 3)
            UI.draw(); return
        end
        n = math.floor(n + 1e-9)
        local p = Players.get(UI.playerName)
        if n > roundMoney(p.balance or 0) then
