...
function drawScreen()
    ...
    local cacheOk = ensureTableCache(mw)
    ...
    if isAdmin or UI.alert or UI.input.active or UI.withdraw.active then
        ...
        if dstActive and _tableBuf and cacheOk then
            pcall(gpu.bitblt, _screenBuf, 1, 1, mw, UI.h, _tableBuf, 1, 1)
        elseif dstActive then
            -- кэш недоступен, но кадр активен: рисуем фон в тот же буфер,
            -- не сбивая активный буфер вызовом blitTable
            paintFeltToActive(mw, UI.h)
            paintRailToActive(mw)
        else
            blitTable(mw)
