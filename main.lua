...
-- Светлее/темнее того же цвета — для лёгкого рельефа на кнопках
-- (иначе плоская заливка почти сливается с тёмным фоном панели)
local function shiftColor(c, delta)
...
function UI.drawButton(b)
    ...
    -- лёгкий бевел (светлый верх / тёмный низ) — кнопка визуально
    -- отделяется от фона, а не сливается плоским пятном цвета
    if b.h >= 3 then
        gpu.setBackground(shiftColor(b.bg, 28))
        gpu.fill(b.x, b.y, b.w, 1, " ")
        gpu.setBackground(shiftColor(b.bg, -32))
        gpu.fill(b.x, b.y + b.h - 1, b.w, 1, " ")
    end
...
local COIN_W, COIN_H = 25, 13
local COIN_Y = 4  -- отступ сверху под бортик стола (строки 2-3 — рамка)
...
function eraseCoinArea(x, y)
    gpu.setBackground(FELT_BASE)
    gpu.fill(x, y - 2, COIN_W, COIN_H + 4, " ")
end
...
function UI.drawMainArea()
    ...
    local HISTORY_Y = coinY + COIN_H + 17  -- см. вывод отступов ниже (держать в синхроне с actionY)
    ...
    if UI.screen == "main" then
        ...
        local maxRows = math.max(3, math.min(10, UI.h - HISTORY_Y - 2))
