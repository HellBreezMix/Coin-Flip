--------------------------------------------------
-- CoinFlip Casino
-- config.lua
--------------------------------------------------

local config = {}

--------------------------------------------------
-- Проект
--------------------------------------------------
config.project = {
    name    = "CoinFlip",
    version = "1.0.0",
    author  = "hellbreez"
}

--------------------------------------------------
-- Администраторы (видят кнопку "Админ-панель")
--------------------------------------------------
config.admins = {
    ["hellbreez"] = true,
    ["Lofland"]   = true
}

--------------------------------------------------
-- Железо (стороны относительно компьютера)
-- 0 = bottom, 1 = top, 2 = north, 3 = south, 4 = west, 5 = east
--------------------------------------------------
config.hardware = {
    -- Транспозер: левый сундук СВЕРХУ (приём предметов для пополнения)
    transposerSide = 1,   -- top

    -- ME Interface: правый сундук СВЕРХУ (выдача предметов / хранение)
    meSide = 1            -- top
}

--------------------------------------------------
-- Валюта
--------------------------------------------------
config.currency = {
    name   = "ЭМ",
    symbol = "ЭМ"
}

--------------------------------------------------
-- Ставки (можно менять в админ-панели)
--------------------------------------------------
config.bet = {
    min = 1,
    max = 1000,
    default = 10
}

--------------------------------------------------
-- Игровые правила
--------------------------------------------------
config.game = {
    winPayout = 2.0   -- выплата за угадывание стороны (1:1, x2 от ставки)
}

--------------------------------------------------
-- Скупка предметов
-- Можно указывать просто число или таблицу { price, label }
-- Админка всё равно перезапишет при добавлении предметов
--------------------------------------------------
config.buyPrices = {
    -- ["customnpcs:npcMoney"] = { price = 1, label = "Деньги" },
    -- ["minecraft:diamond"]   = { price = 50, label = "Алмаз" },
}

--------------------------------------------------
-- Пути данных
--------------------------------------------------
config.paths = {
    root     = "/CoinFlip/",
    data     = "/CoinFlip/data/",
    players  = "/CoinFlip/data/players.db",
    settings = "/CoinFlip/data/settings.db",
    history  = "/CoinFlip/data/history.db",
    log      = "/CoinFlip/data/log.txt"
}

--------------------------------------------------
-- Цвета (тёмная тема)
--------------------------------------------------
config.colors = {
    background   = 0x0A0A0A,
    panel        = 0x141414,
    panelLight   = 0x1E1E1E,
    header       = 0x1A1A2E,

    text         = 0xE0E0E0,
    textDark     = 0x888888,
    textGold     = 0xFFD700,
    textGreen    = 0x00CC66,
    textRed      = 0xFF4444,
    textBlue     = 0x55AAFF,

    button       = 0x2A2A2A,
    buttonHover  = 0x3A3A3A,
    buttonGreen  = 0x00C662,
    buttonRed    = 0xE04040,
    buttonBlue   = 0x4477E0,
    buttonYellow = 0xE0BB00,

    tableGreen   = 0x0B5C3A,

    -- монета
    coinGold     = 0xD4AF37,
    coinWin      = 0x1E8A3A,
    coinLose     = 0xA22626
}

--------------------------------------------------
-- Размеры UI
--------------------------------------------------
config.ui = {
    sidebarWidth = 52,
    headerHeight = 1
}

return config
